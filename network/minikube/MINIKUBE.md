...
kubectl cp ./network/minikube/configtx.yaml $(kubectl get pods -o=name | grep storage-setup | sed "s/^.\{4\}//"):/host/files
kubectl cp ./network/minikube/config.yaml $(kubectl get pods -o=name | grep storage-setup | sed "s/^.\{4\}//"):/host/files
...
kubectl cp ./fabric-samples/bin $(kubectl get pods -o=name | grep storage-setup | sed "s/^.\{4\}//"):/host/files
```
...
kubectl exec -it $(kubectl get pods -o=name | grep cli-peer0-carrier-deployment | sed "s/^.\{4\}//") -- bash -c 'peer chaincode invoke -C mainchannel -n resources -c '\''{"Args":["Create","Mainframe Boards","1"]}'\'' -o orderer0-service:7050 --tls --cafile=/etc/hyperledger/orderers/msp/tlscacerts/orderers-ca-service-7054.pem'
sleep 5
